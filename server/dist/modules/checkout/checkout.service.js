...
const paystack_1 = require("@/infra/payment/paystack");
const dpo_1 = require("@/infra/payment/dpo");
...
class CheckoutService {
    constructor() { }
    async createCheckoutSession(cart, userId, provider = "stripe", paymentDetails, couponCode) {
        ...
        const totalAmountBeforeDiscount = cart.cartItems.reduce((acc, item) => {
        ...
        if (couponCode) {
            ...
            try {
                ...
                throw new AppError_1.default(400, error.message || "Invalid coupon code");
            }
        }
        ...
        if (provider === "dpo") {
            // Create DPO Token
            const dpoResponse = await dpo_1.dpoService.createToken({
                amount: finalAmount,
                currency: "USD",
                email,
            ...
            if (paymentDetails?.phoneNumber && paymentDetails?.mno) {
                try {
                    await dpo_1.dpoService.chargeMobile({
        ...
        if (provider === "paystack") {
            ...
            const session = await paystack_1.paystackService.initializeTransaction({
                email,
