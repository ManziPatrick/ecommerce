...
class CartService {
    ...
    async logCartEvent(cartId, eventType, userId) {
        ...
        console.log("🔍 [CART SERVICE] userId:", userId);
        await database_config_1.default.cartEvent.create({
            data: {
                userId,
                cartId,
                eventType,
    ...
    async getAbandonedCartMetrics(startDate, endDate) {
        const cartEvents = await database_config_1.default.cartEvent.findMany({
            where: {
                timestamp: {
                    gte: startDate,
                    lte: endDate,
    ...
    async addToCart(variantId, quantity, userId, sessionId) {
    ...
    async mergeCartsOnLogin(sessionId, userId) {
