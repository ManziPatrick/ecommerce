...
class VariantController {
    constructor(variantService) {
        ...
        this.getAllVariants = (0, asyncHandler_1.default)(async (req, res) => {
            const { variants, totalResults, totalPages, currentPage, resultsPerPage, } = await this.variantService.getAllVariants(req.query, req.user?.id, req.user?.role);
        ...
        this.getRestockHistory = (0, asyncHandler_1.default)(async (req, res) => {
            const { id: variantId } = req.params;
            const { page = "1", limit = "10" } = req.query;
            const { restocks, totalResults, totalPages, currentPage, resultsPerPage, } = await this.variantService.getRestockHistory(variantId, parseInt(page), parseInt(limit), req.user?.id, req.user?.role);
            (0, sendResponse_1.default)(res, 200, {
                data: {
                    restocks,
                    totalResults,
        ...
        this.restockVariant = (0, asyncHandler_1.default)(async (req, res) => {
            const { id: variantId } = req.params;
            ...
            const { restock, isLowStock } = await this.variantService.restockVariant(variantId, parsedQuantity, notes, req.user?.id);
            (0, sendResponse_1.default)(res, 200, {
        ...
        this.deleteVariant = (0, asyncHandler_1.default)(async (req, res) => {
            const { id: variantId } = req.params;
            await this.variantService.deleteVariant(variantId, req.user?.id, req.user?.role);
