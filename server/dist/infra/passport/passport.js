...
function configurePassport() {
    ...
    passport_1.default.use(new passport_google_oauth20_1.Strategy({
        ...
        try {
            let user = await database_config_1.default.user.findUnique({
                where: { email: profile.emails[0].value },
            });
            if (user) {
                if (!user.googleId) {
                    user = await database_config_1.default.user.update({
                        where: { email: profile.emails[0].value },
                        data: {
                            googleId: profile.id,
                ...
                user = await database_config_1.default.user.create({
                    data: {
