...
const optionalAuth = async (
  ...
  try {
    ...
    // Silently handle token expiration - this is expected behavior
    if (error.name === 'TokenExpiredError') {
      console.log("🔍 [OPTIONAL AUTH] Token expired (this is normal)");
    } else if (error.name === 'JsonWebTokenError') {
      console.log("🔍 [OPTIONAL AUTH] Invalid token format");
    } else {
      console.log("🔍 [OPTIONAL AUTH] Auth error:", error.message);
    }
