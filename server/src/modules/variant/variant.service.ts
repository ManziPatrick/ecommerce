...
export class VariantService {
  ...
  async getAllVariants(queryString: Record<string, any>, userId?: string, userRole?: string) {
    ...
    if (userRole === "VENDOR") {
      const shop = await (prisma as any).shop.findFirst({ where: { ownerId: userId } });
      if (shop) {
  ...
  async getRestockHistory(
    ...
    if (userRole === "VENDOR") {
      ...
      const shop = await (prisma as any).shop.findFirst({ where: { ownerId: userId } });
      if (!shop || (variant.product as any).shopId !== shop.id) {
        throw new AppError(403, "You are not authorized to view this restock history");
      }
    }
  ...
  async createVariant(data: {
    ...
    if (userRole === "VENDOR") {
      ...
      const shop = await (prisma as any).shop.findFirst({ where: { ownerId: userId } });
      if (!shop || (product as any).shopId !== shop.id) {
        throw new AppError(403, "You can only create variants for your own products");
      }
    }
  ...
  async updateVariant(
    ...
    if (userRole === "VENDOR") {
      const shop = await (prisma as any).shop.findFirst({ where: { ownerId: userId } });
      if (!shop || (existingVariant.product as any).shopId !== shop.id) {
        throw new AppError(403, "You are not authorized to update this variant");
      }
    }
  ...
  async deleteVariant(variantId: string, userId?: string, userRole?: string) {
    const variant = await this.variantRepository.findVariantById(variantId);
    if (!variant) {
      throw new AppError(404, "Variant not found");
    }
    ...
    if (userRole === "VENDOR") {
      const shop = await (prisma as any).shop.findFirst({ where: { ownerId: userId } });
      if (!shop || (variant.product as any).shopId !== shop.id) {
        throw new AppError(403, "You are not authorized to delete this variant");
      }
    }
