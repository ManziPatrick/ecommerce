...
export class ProductService {
  ...
  async createProduct(data: {
    ...
    if (userRole === "VENDOR") {
      ...
      if (!shop) {
        throw new AppError(403, "You do not have a shop to create products in");
      }
      if (productData.shopId && productData.shopId !== shop.id) {
        throw new AppError(403, "You can only create products for your own shop");
      }
    ...
    variants.forEach((variant, index) => {
      ...
      if (variant.discountPrice !== undefined && variant.discountPrice < 0) {
        throw new AppError(
          400,
          `Variant at index ${index} must have a non-negative discount price`
        );
      }
      if (variant.stock < 0) {
        throw new AppError(
    ...
    try {
      ...
      if (error.code === "P2002") {
        ...
        if (target.includes("name")) {
          throw new AppError(400, "A product with this name already exists");
        }
      }
      if (error.message?.includes("Transaction already closed")) {
        throw new AppError(
          500,
          "The operation took too long. Please try again with fewer variants or check your connection."
        );
      }
      throw error;
    }
  ...
  async bulkCreateProducts(file: Express.Multer.File, userId?: string, userRole?: string) {
    if (!file) {
