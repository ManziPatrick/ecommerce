...
export class ProductController {
  ...
  getAllProducts = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      const {
        ...
        currentPage,
        resultsPerPage,
      } = await this.productService.getAllProducts(queryString);
  ...
  createProduct = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      let imageResults: ({ url: string; public_id: string } | null)[] = [];
      if (files.length > 0) {
        try {
          ...
          if (successCount === 0) {
            throw new AppError(400, "Failed to upload all images to Cloudinary");
          }
        ...
        ? await Promise.all(
          parsedVariants.map(async (variant: any, index: number) => {
            ...
            if (variant.discountPrice !== undefined && typeof variant.discountPrice !== "number") {
              throw new AppError(
                400,
                `Variant at index ${index} discountPrice must be a number`
              );
            }
            if (variant.stock < 0) {
              throw new AppError(
