...
const ProductImageGallery: React.FC<ProductImageGalleryProps> = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex ${isFullScreen ? "h-full" : ""
          } flex-col md:flex-row gap-6`}
      >
        {/* Thumbnails */}
        <div
          className={`flex md:flex-col gap-3 ${isFullScreen
        ...
        >
          {images.map((img, index) => (
            <button
              ...
              className={`relative border-2 rounded-xl p-1 transition-all duration-200 ${selectedImage === img
            ...
            >
              <div className="relative w-20 h-20">
                <Image
                  src={getCloudinaryUrl(img, "w_200,h_200,c_fill,f_auto,q_auto")}
        ...
        <div
          className={`relative flex-1 ${isFullScreen ? "flex items-center justify-center" : ""
            }`}
        >
          {/* Navigation Arrows */}
          ...
          <div className="absolute top-4 right-16 flex gap-2 z-10">
            <button
              ...
              className={`bg-white bg-opacity-80 rounded-full p-2 shadow-md transition-all ${isZoomed ? "bg-indigo-100 text-indigo-600" : "hover:bg-gray-100"
                }`}
          ...
          {/* Main Image */}
          <div
            className={`flex items-center justify-center ${isFullScreen ? "h-full" : "bg-gray-50 rounded-2xl p-6"
              } overflow-hidden`}
          ...
          >
            <div
              className={`relative ${isFullScreen ? "max-h-full max-w-full" : "h-[500px] w-full"
                } overflow-hidden rounded-xl`}
            >
              <Image
                src={getCloudinaryUrl(selectedImage)}
                alt={name}
                fill
                sizes="(max-width: 768px) 100vw, 50vw"
