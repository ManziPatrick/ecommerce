...
import { useVerifyDPOPaymentMutation } from "@/app/store/apis/PaymentApi";
import { useCreateOrderMutation } from "@/app/store/apis/OrderApi";
...
const SuccessPage = () => {
  ...
  const [verifyPayment] = useVerifyDPOPaymentMutation();
  ...
  React.useEffect(() => {
    const handleVerification = async () => {
      if (transToken && verificationStatus === "idle") {
        setVerificationStatus("verifying");
        try {
          const verifyResult = await verifyPayment({ transToken }).unwrap();

          if (verifyResult.data.Result === "000") {
            if (cartId) {
              setStatusMessage("Payment verified. Creating order...");
              await createOrder({ cartId }).unwrap();
              setStatusMessage("Order created successfully!");
              // Optionally update URL to remove token and add orderId
              // But for now, just showing success checkmark is enough or relying on existing UI config
            } else {
              setStatusMessage("Payment verified successfully!");
            }
            setVerificationStatus("success");
          } else {
            setStatusMessage(`Payment verification failed: ${verifyResult.data.ResultExplanation}`);
    ...
    if (transToken) {
      handleVerification();
    } else if (provider === "stripe" && cartId && !orderId) {
      // For Stripe, success_url means payment worked (basic assumption).
      // Trigger order creation immediately if not done yet.
      const createStripeOrder = async () => {
        if (verificationStatus === "idle") {
          ...
          try {
            ...
            setStatusMessage("Failed to create order. Please contact support.");
            setVerificationStatus("error");
          }
        }
      };
  ...
  }, [transToken, cartId, verifyPayment, createOrder, provider]);
  ...
  // Override config if we are in verification mode
  if (transToken) {
    ...
    if (verificationStatus === "error") {
      return (
        <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-white text-center">
          ...
          <Link href="/cart" className="bg-indigo-600 text-white px-6 py-2 rounded-md hover:bg-indigo-700">
            Return to Cart
          </Link>
        </div>
      );
    ...
    // If success, we fall through to the normal SuccessPage render, 
    // possibly mocking the 'type=order' config if we didn't have it originally.
  }
