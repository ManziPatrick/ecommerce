...
const OrderStatus = ({ order }) => {
  ...
  const getStatusDate = (status) => {
    switch (status) {
      ...
      case "RETURNED":
      case "REFUNDED":
        return order.transaction?.updatedAt ? formatDate(order.transaction.updatedAt) : "N/A";
      default:
        return "N/A";
    }
  };

  // Use order.status if transaction is null
    ...
    >
      ...
      <div className="flex flex-wrap justify-between relative mt-6">
        ...
        <div
          ...
          style={{
            zIndex: 2,
            width: `${((currentStep - 1) / Math.max(statusSteps.length - 1, 1)) * 100
              }%`,
          }}
        ></div>
        {statusSteps.map((status, index) => (
          <motion.div
            key={status}
            className={`flex flex-col items-center z-10 w-${statusSteps.length === 2 ? "1/2" : `1/${statusSteps.length}`
              }`}
            initial={{ scale: 0.8 }}
            animate={{ scale: 1 }}
          ...
          >
            <div
              className={`w-12 h-12 rounded-md flex items-center justify-center mb-2 ${currentStep >= index + 1
